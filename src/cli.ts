...
import { buildThinkingState, computeThinkingIncrement, readThinkingState, saveThinkingState } from './run/thinking.js';
...
const runCommand = program
  .command('run')
  .description('Start a new run or continue an existing conversation')
  .argument('[args...]', 'prompt or run_id + prompt')
  ...
  .option('--thinking <mode>', 'thinking effort (extended or standard)', 'extended')
  .option('--json', 'Output machine-readable JSON', false);

if (DEV_MODE) {
  runCommand.option('--browser <browser>', 'chrome or firefox', 'chrome');
  runCommand.option('--firefox-profile <path>', 'Firefox profile path (defaults to ~/.oracle/firefox)');
  runCommand.option('--firefox-app <path>', 'Firefox app bundle or binary path (Developer Edition / Nightly)');
  runCommand.option('--poll-ms <ms>', 'Polling interval for streaming', String(DEFAULT_POLL_MS));
  runCommand.option('--timeout-ms <ms>', 'Max wait time for completion', String(DEFAULT_TIMEOUT_MS));
  runCommand.option('--max-attempts <n>', 'Max attempts', String(DEFAULT_MAX_ATTEMPTS));
}

runCommand.action(async (args, options) => {
    const { runId, prompt, existing } = await resolveRunInvocation(args, options);
    const runDirPath = runDir(runId, options.runsRoot);
    await ensureDir(runDirPath);

    const promptHash = crypto.createHash('sha256').update(prompt).digest('hex');
    const baseUrl = resolveBaseUrl(options.baseUrl);
    const pollMs = DEV_MODE && options.pollMs ? Number(options.pollMs) : DEFAULT_POLL_MS;
    const timeoutMs = DEV_MODE && options.timeoutMs ? Number(options.timeoutMs) : DEFAULT_TIMEOUT_MS;
    const maxAttempts = DEV_MODE && options.maxAttempts ? Number(options.maxAttempts) : DEFAULT_MAX_ATTEMPTS;
    ...
    if (existing) {
      ...
      if (DEV_MODE && options.browser) {
        ...
        config.firefoxApp = browser === 'firefox' ? resolveFirefoxApp(options.firefoxApp) : undefined;
      }
      ...
      const browser = DEV_MODE && options.browser === 'firefox' ? 'firefox' : 'chrome';
...
program
  .command('thinking')
  .description('Print thinking output for a run')
  .argument('<run_id>', 'run id')
  .option('--runs-root <dir>', 'Runs root directory', defaultRunsRoot())
  .option('--full', 'Return full thinking output', false)
  .action(async (runId, options) => {
    const runDirPath = runDir(runId, options.runsRoot);
...
async function resolveRunInvocation(
  ...
  const promptFromOptions = await loadPromptFromOptions(options.prompt, options.promptFile);
  let prompt = promptFromOptions ?? (promptParts.length ? promptParts.join(' ') : null);
  if (!prompt) {
...
async function loadPromptFromOptions(prompt?: string, promptFile?: string): Promise<string | null> {
  if (prompt !== undefined) {
