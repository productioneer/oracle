...
export async function applyFocusStrategy(
  options: FocusStrategyOptions,
): Promise<FocusStatus | undefined> {
  ...
  const appName =
    options.appName ??
    (options.browser === "firefox" ? "Firefox" : "Google Chrome");
...
export async function runFirefoxSetupPhase(
  ...
  if (!pid) {
    return {
      focus: { state: "visible", reason: "pid-missing" },
      attempts: 0,
      elapsedMs: 0,
    };
  }
  const trusted = await waitForTrustedFirefoxWindow(appName, pid, logger, {
    timeoutMs: 1_000,
  });
  if (!trusted) {
    logger?.("[focus] firefox window mismatch; skipping suppression");
    return {
      focus: { state: "visible", reason: "window-mismatch" },
      attempts: 0,
  ...
  while (Date.now() - start < FIREFOX_SETUP_MAX_MS) {
    attempts += 1;
    const focusAttempt = await suppressFirefoxFocusMac(appName, pid, logger, {
      delayMs: 0,
    });
    focus = mergeFocusStatus(focus, focusAttempt);
    const resizeAttempt = await resizeFirefoxWindow(
      appName,
      pid,
      "setup",
      logger,
...
export async function resizeFirefoxWindow(
  ...
  if (result.ok) {
    ...
    logger?.(
      `[focus] firefox ${phase} window size ${size.width}x${size.height}`,
    );
...
export async function waitForTrustedFirefoxWindow(
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    if (title.ok) {
      ...
      if (
        lastTitle &&
        lastTitle.toLowerCase().includes(FIREFOX_HOME_TITLE.toLowerCase())
      ) {
        return true;
      }
    }
      ...
      (error, stdout, stderr) => {
        if (error) {
          ...
          resolve({
            ok: false,
            stdout: stdout ?? "",
            error: message || "osascript-failed",
          });
          return;
        }
...
export function classifyAppleScriptError(raw: string): FocusNeeds | undefined {
  ...
  if (
    ...
    if (
      lower.includes("system events") ||
      lower.includes("apple events") ||
...
export function resolveFocusStatus(attempts: FocusAttempt[]): FocusStatus {
  for (const attempt of attempts) {
    if (
      (attempt.action === "hide" || attempt.action === "minimize") &&
      attempt.ok
    ) {
  ...
  const needsUser = [...attempts]
    .reverse()
    .find((attempt) => attempt.needsUser)?.needsUser;
  return {
    state: "visible",
...
function mergeFocusStatus(
  current: FocusStatus,
  incoming: FocusStatus,
): FocusStatus {
...
async function getWindowState(
  ...
  return {
    ok: false,
    reason: value ? `unexpected-window:${value}` : "unexpected-window",
  };
}
...
async function parkFirefoxWindowOffscreen(
  appName: string,
  pid: number,
  logger?: Logger,
): Promise<boolean> {
  ...
  if (!result.ok) {
    logger?.(
      `[focus] firefox park offscreen failed: ${result.error ?? "unknown"}`,
    );
    return false;
  }
  const output = result.stdout.trim().toLowerCase();
