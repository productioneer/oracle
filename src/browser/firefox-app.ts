...
export function resolveFirefoxApp(
  appPath?: string,
): FirefoxAppConfig | undefined {
...
function validateFirefoxAppPath(
  rawPath: string,
  source: FirefoxAppConfig["source"],
): FirefoxAppConfig {
  ...
  if (!fs.existsSync(normalized.executablePath)) {
    throw new Error(
      `Firefox executable missing at ${normalized.executablePath}`,
    );
  }
  return { ...normalized, source };
}

function buildFirefoxAppConfig(
  appPath: string,
): Omit<FirefoxAppConfig, "source"> {
  const appName = path.basename(appPath, ".app");
  const executablePath = path.join(appPath, "Contents", "MacOS", "firefox");
  return { appPath, appName, executablePath };
}

export function normalizeFirefoxAppPath(
  rawPath: string,
): Omit<FirefoxAppConfig, "source"> | null {
