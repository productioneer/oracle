...
import { DEFAULT_BASE_URL, ensureChatGptReady, ensureWideViewport, getThinkingContent, navigateToChat } from '../browser/chatgpt.js';
...
export async function readThinkingContent(config: RunConfig): Promise<string> {
  ...
  try {
    if (config.browser === 'chrome') {
      ...
      const active = config.debugPort ? await isChromeDebugPortActive(config.debugPort) : false;
...
async function waitForConversationContent(page: import('puppeteer').Page, timeoutMs: number): Promise<void> {
  ...
  while (Date.now() - start < timeoutMs) {
    const hasThinkingUI = await page.evaluate(() => {
      const elements = Array.from(document.querySelectorAll('button, span')) as HTMLElement[];
      const thoughtEl = elements.find((el) => {
        const text = (el.textContent || '').trim();
        return /^thought for /i.test(text) && text.length < 50;
      });
      if (thoughtEl) return true;
      const sidebars = Array.from(document.querySelectorAll('.bg-token-sidebar-surface-primary')) as HTMLElement[];
...
async function isOracleChromeRunning(userDataDir: string): Promise<boolean> {
  ...
  return lines.some((line) => line.includes('Google Chrome') && line.includes(`--user-data-dir=${normalized}`));
}
